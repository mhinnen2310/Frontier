...
Frontier is currently undergoing the 60-sprint Master Remediation & Expansion Roadmap. Release Trains A–B cover the audited foundation, recoverable settlement founding/membership, complete district management and physically activated specialization balance. Train C is underway with building validation/registration and bounded physical route validation finished; infrastructure failure integration is next. The Atlas, always-attackable cargo/crime, map walls, local web map and unified history expansion are not yet complete. See [IMPLEMENTATION_STATUS.md](IMPLEMENTATION_STATUS.md) for the evidence-backed status; no presence-only feature is labeled complete.
