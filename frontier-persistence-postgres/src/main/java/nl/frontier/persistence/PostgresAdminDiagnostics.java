...
public final class PostgresAdminDiagnostics implements AdminDiagnostics {
  ...
  private static final Map<String, String> VIEWERS =
      Map.of(
          "settlement",
              "SELECT jsonb_build_object('city',to_jsonb(c),'treasury',(SELECT balance_minor FROM accounts WHERE owner_type='CITY' AND owner_id=c.id),'members',(SELECT count(*) FROM city_members WHERE city_id=c.id),'claims',(SELECT count(*) FROM city_claims WHERE city_id=c.id),'buildings',(SELECT count(*) FROM city_buildings WHERE city_id=c.id),'districts',(SELECT count(*) FROM districts WHERE city_id=c.id),'populationState',(SELECT to_jsonb(p) FROM city_population_state p WHERE p.city_id=c.id))::text FROM cities c WHERE c.id=?",
          "influence",
